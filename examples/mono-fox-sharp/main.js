(() => {
  ...
  // Sharp/HiDPI rendering: backing store at native pixel density,
  // logical drawing coords stay W x H. CSS still scales to container.
  let DPR = 1;
  ...
  configureCanvas();
  addEventListener('resize', configureCanvas);

  const UI_FONT = 'system-ui, -apple-system, "Inter", "Helvetica Neue", "Segoe UI", sans-serif';
  ...
  function drawHaze() {
    if (tunnel.active) return;
    const horizon = project(0, 0, 600);
    if (!horizon) return;
    const yh = horizon[1];
    // Soft horizontal band centered on the horizon, fading out top/bottom
    const grad = ctx.createLinearGradient(0, yh - 50, 0, yh + 90);
